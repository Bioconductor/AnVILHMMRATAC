...
```{r install}
pkgs = c("Bioconductor/AnVIL", "DiffBind", "rGREAT", "ChIPseeker", i
...
sub_tbl <- as.data.frame(part_tbl[7:12, -3])
```

```{r diffbind}
dba <- dba(sapleSheet = sub_tbl)
count <- dba.count(dba, summits=500)
contrast <- dba.contrast(count, categorizes = DBA_CONDITION)
